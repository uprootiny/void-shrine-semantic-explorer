...
(defroutes entropy-gallery-routes
  ...
  (GET "/api/entropy-sample" request
    (let [source (get-in request [:params :source])
          source-key (keyword (or source ""))
      ...
      (if source-data
        ...
        (-> (json/write-str {:error "Source not found" :requested source :available-sources (keys (sources/get-all-sources))}) response/response (response/status 404)))))
